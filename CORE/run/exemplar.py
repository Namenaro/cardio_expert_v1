...
class Exemplar:
    ...
    def create_snapshot_from_step(self, step_num: int, form: Form) -> Optional['Exemplar']:
        """
        Создает глубокую копию экземпляра, удаляя все точки, установленные на данном шаге и последующих,
        а также все параметры экземпляра.

        :param step_num: номер шага (начиная с 0), начиная с которого нужно удалить точки
        :param form: объект формы, содержащий информацию о шагах и их целевых точках
        :return: новый экземпляр Exemplar с удаленными точками и очищенными параметрами,
                 или None, если шаг не найден
        """
        import copy

        # Проверяем, что шаг с таким номером существует в форме
        if step_num < 0 or step_num >= len(form.steps):
            logger.error(f"Шаг {step_num} не найден в форме. Всего шагов: {len(form.steps)}")
            return None

        # Создаем глубокую копию текущего экземпляра
        snapshot = copy.deepcopy(self)

        # Очищаем ВСЕ параметры в снимке
        snapshot._parameters.clear()
        snapshot.evaluation_result = None

        # Собираем имена точек, которые нужно удалить (начиная с указанного шага)
        points_to_remove = []
        for step in form.steps[step_num:]:  # Исправлено: step_num, а не step_num - 1
            if step.target_point and step.target_point.name:
                points_to_remove.append(step.target_point.name)

        # Удаляем точки из копии
        for point_name in points_to_remove:
            if point_name in snapshot._points:
                del snapshot._points[point_name]
                logger.debug(f"Точка {point_name} удалена из снимка")

        logger.info(f"Создан снимок экземпляра {self.id} начиная с шага {step_num}. "
                    f"Удалено точек: {len(points_to_remove)} из {len(self._points)}. "
                    f"Все параметры ({len(self._parameters)}) очищены.")
