...
class RStepsListCreator:
    ...
    def from_db_form(self, form: Form, schema: Schema) -> List[RStep]:
        ...
        """
        self.form = form
        self.schema = schema  # сохраняем схему для использования в _init_rstep

        # Инициализуирем шаги
        self.rsteps: List[RStep] = []
        for step_bd in form.steps:
            r_step = self._init_rstep(step_bd)  # убираем schema из параметров
            self.rsteps.append(r_step)
        return self.rsteps

    def _init_rstep(self, step: Step):
        """
