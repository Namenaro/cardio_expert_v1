...
def select_form_from_dialog(forms: List[Form]) -> Tuple[Optional[int], bool]:
    """
    Отображает диалоговое окно для выбора формы из списка или создания новой формы.
    Диалог является модальным и блокирует интерфейс до закрытия
        - При двойном клике по элементу списка форма выбирается автоматически
        - При закрытии диалога через крестик возвращается (None, False)
        - При нажатии "Создать новую форму" возвращается (None, True)
        - При выборе существующей формы возвращается (form_id, False)

    Args:
        forms (List[Form]): Список объектов Form для отображения в диалоге.

    Returns:
            - первый элемент: ID выбранной формы ( если пользователь выбрал существующую форму)
            - второй элемент: нужно ли создавать новую форму
    """
