...
class OutputParamsWidget(QWidget):
    """Виджет для редактирования выходных параметров объекта"""

    def __init__(self, parent=None):
        super().__init__(parent)
        self._output_params: List[ClassOutputParam] = []
        self._current_values: List[ObjectOutputParamValue] = []
        self.setup_ui()
    ...
    def setup_ui(self):
        ...
        # Заголовок
        self.title_label = QLabel("Выходные параметры:")
        self.title_label.setStyleSheet("font-weight: bold; color: #00aa55;")
    ...
    def load_output_params(self,
                          output_params: List[ClassOutputParam],
                          form_parameters: List[Parameter]):
        """
        Загрузка списка выходных параметров и параметров формы

        Args:
            outut_params: Список выходных параметров класса
            form_parameters: Список параметров формы
        """
        self._output_params = output_params
        self.table_widget.setRowCount(len(output_params))

        for row, output_param in enumerate(output_params):
            # Название параметра класса (нередактируемое)
            name_item = QTableWidgetItem(output_param.name if output_param.name else "")
            ...
            current_value = self._get_current_param_value(output_param.id)
            ...
            type_item = QTableWidgetItem(output_param.data_type if output_param.data_type else "")
            type_item.setFlags(type_item.flags() & ~Qt.ItemFlag.ItemIsEditable)
            self.table_widget.setItem(row, 2, type_item)

            comment_item = QTableWidgetItem(output_param.comment if output_param.comment else "")
            comment_item.setFlags(type_item.flags() & ~Qt.ItemFlag.ItemIsEditable)
            self.table_widget.setItem(row, 3, comment_item)

    def _get_current_param_value(self, output_param_id: int) -> Optional[ObjectOutputParamValue]:
        """Получить текущее значение параметра"""
        for value in self._current_values:
            if value.output_param_id == output_param_id:
                return value
        return None
    ...
    def _on_param_selected(self, index: int):
        ...
        if row < len(self._output_params):
            output_param = self._output_params[row]

            # Обновляем или создаем значение
            value = self._get_current_param_value(output_param.id)
            if not value:
                value = ObjectOutputParamValue(
                    id=None,
                    object_id=None,
                    output_param_id=output_param.id,
    ...
    def validate(self) -> bool:
        """
        Проверка, что для всех ввходных параметров выбраны параметры формы
        ...
        # Проверяем каждый выходной параметр
        for output_param in self._output_params:
            if output_param.id is None:
                continue
    ...
    def get_validation_errors(self) -> str:
        ...
        for output_param in self._output_params:
            ...
            value = self._get_current_param_value(output_param.id)
            if not value or value.parameter_id is None:
                param_name = output_param.name if output_param.name else f"ID:{output_param.id}"
                errors.append(param_name)

        params_list = "\n• " + "\n• ".join(errors)
        error_str = f"Для следующих выходных параметров класса необходимо выбрать параметры формы:\n{params_list}"

        return error_str

    def get_output_param_values(self) -> List[ObjectOutputParamValue]:
        """Получить значения выходных параметров"""
        return self._current_values.copy()

    def load_current_values(self, values: List[ObjectOutputParamValue]):
            ...
            for row in range(self.table_widget.rowCount()):
                ...
                if combo_widget:
                    if id_item := self.table_widget.item(row, 0):
                        ...
                        # Находим соответствующий output_param
                        output_param = next((p for p in self._output_params
    ...
    def clear(self):
        ...
        self.title_label.setText("Выходные параметры:")
